...
proc target_appear {time_stamp} {
    ...
    set opt(target_on) 1
    $target color "red"
    puts "At $time_stamp: Target YES!";         # test
}
...
proc fixed_sensors_detect {time_stamp} {
    ...
    for {set i 0} {$i < $opt(nfnode)} {incr i} {
        ...
        puts "signal = $signal"; # test
        set local_proba [get_local_detec_proba $signal $dist $time_stamp]
        set false_rate [get_false_alarm_rate $signal $time_stamp]
        set pba [expr $pba * $local_proba]
        set pb_a [expr $pb_a * $false_rate]
    }
    puts "pba = $pba pb_a = $pb_a";          # test
    set pba [expr $opt(PA) * $pba]
    set pb_a [expr (1 - $opt(PA)) * $pb_a]
    puts "Then, pba = $pba pb_a = $pb_a"; # test
