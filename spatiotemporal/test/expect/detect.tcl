...
set opt(pdi) $normal(1.00);  # The Local Detection Probability
...
#set opt(threshold_expect) [expr round($opt(nfnode) * $opt(pdi))]; # Expect of local decition count
#set opt(threshold_expect) [expr int($opt(nfnode) * $opt(pdi)) + 1]
set opt(threshold_expect) [expr int($opt(nfnode) * $opt(pdi))]
if {!$opt(threshold_expect)} {
    set opt(threshold_expect) 1
}
#set opt(threshold_expect) [expr round($opt(nfnode) / 2.0)]
#puts "threshold_expect $opt(threshold_expect)"
...
proc fixed_sensors_detect {time_stamp} {
    global opt fnode fdists lambda pdi pfi
    for {set i 0} {$i < $opt(nfnode)} {incr i} {
        $fnode($i) color "black"
    }
    set plc 0;              # positive local count
    for {set i 0} {$i < $opt(nfnode)} {incr i} {
        set dist $fdists($i)
...
proc compute_lambda {dist} {
    ...
    #set sigma2 $opt(noise_var)
    set sigma $opt(noise_std)
    #puts $opt(tmpfile) "avg: $opt(noise_avg)"
    #puts $opt(tmpfile) "var: $opt(noise_var)"
    set lambda [expr $e_s + $mu - $opt(sensitivity) * $sigma]
    return $lambda
}
...
for {set i 0} {$i < $opt(nfnode)} {incr i} {
    ...
    #puts $opt(tmpfile) "Node $i: dist = $fdists($i)"
    #puts $opt(tmpfile) "Node $i: lambda = $lambda($i)"
    #set pdi($i) [get_local_detec_proba $lambda($i) $dist 0]
    set pfi($i) [get_false_alarm_rate $lambda($i) 0]
    #puts $opt(tmpfile) "Node $i: pdi = $pdi($i)"
    #puts $opt(tmpfile) "Node $i: pfi = $pfi($i)"
    $fnode($i) set X_ $xf
    $fnode($i) set Y_ $yf
    $fnode($i) set Z_ 0
