...
set opt(major_threshold) [expr int($opt(nfnode)/2) + 1]
...
proc fixed_sensors_detect {time_stamp} {
    ...
    #puts $opt(tmpfile) "The target is $opt(target_on)"
    set ac 0
    set nc 0
    for {set dt 0} {$dt < $opt(decision_time)} {incr dt} {
        ...
        if {$is_alert >= $opt(major_threshold)} {
            incr ac
    ...
    #puts $opt(tmpfile) "ac = $ac (/$opt(decision_threshold))"
    if {$ac >= $opt(decision_threshold)} {
        if {$opt(target_on)} {
            incr opt(true_alarm)
        } else {
    ...
    #puts $opt(tmpfile) "true_alarm: $opt(true_alarm)"
}
