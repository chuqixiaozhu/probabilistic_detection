...
set opt(noise_var) 0.5; # Noise variance
...
proc fixed_sensors_detect {time_stamp} {
    ...
    for {set dt 0} {$dt < $opt(decision_time)} {incr dt} {
        ...
        for {set i 0} {$i < $opt(nfnode)} {incr i} {
            ...
            set pba [get_local_detec_proba $signal $dist $time_stamp]
            set pb_a [get_false_alarm_rate $signal $time_stamp]
            set PAPBA [expr $PAPBA * $pba]
            set P_APB_A [expr $P_APB_A * $pb_a]
            set p_ba [expr 1.0 - $pba]
            set p_b_a [expr 1.0 - $pb_a]
            set PAP_BA [expr $PAP_BA * $p_ba]
            set P_AP_B_A [expr $P_AP_B_A * $p_b_a]
        }
        set PAPBA [expr $opt(PA) * $PAPBA]
        set P_APB_A [expr (1 - $opt(PA)) * $P_APB_A]
        set PAB [expr $PAPBA / ($PAPBA + $P_APB_A)]
        set PAP_BA [expr $opt(PA) * $PAP_BA]
        set P_AP_B_A [expr (1 - $opt(PA)) * $P_AP_B_A]
        set P_A_B [expr $P_AP_B_A / ($P_AP_B_A + $PAP_BA)]
        if {$PAB >= $P_A_B} {
            incr ac
