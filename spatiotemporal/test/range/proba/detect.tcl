...
set opt(S_0) 5;                             # Maximum of source signal
...
proc fixed_sensors_detect {time_stamp} {
    ...
    for {set dt 0} {$dt < $opt(decision_time)} {incr dt} {
        ...
        if {$Q >= $opt(phi)} {
            incr ac
            if {$ac >= [expr $opt(decision_time)/2 + 1]} {
            #if {$ac >= 1} {}
                break
            }
        }
...
proc system_probas {} {
    ...
    #set presences [expr $opt(PA) * $opt(stop) * $opt(decision_time)]
    #set absences [expr $opt(stop) * $opt(decision_time) - $presences]
