...
for i in range(count):
    ...
    for round_num in range(1, 10, 2):
        subprocess.call(['ns', 'detect.tcl', str(round_num), result_file])

# Process the results
rf = open(result_file, 'r')
pd_file = open('round_num-pro_vs_pd', 'w')
pf_file = open('round_num-pro_vs_pf', 'w')
pd = dict()
pf = dict()
for line in rf:
    results = line.split()
