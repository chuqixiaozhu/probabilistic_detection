...
set opt(nfnode) 11                        ;# number of fixed nodes
...
set opt(radius_range) 3;    # Times of radius

if {0 < $argc} {
    #set opt(nfnode) [lindex $argv 0]
    set opt(PA) [lindex $argv 0]
...
proc deploy_sensor {x_ y_ center_x center_y radius} {
    global opt
    upvar 1 $x_ x
    upvar 1 $y_ y

    set dist [distance_xy $x $y $center_x $center_y]
    while {$dist > $radius} {
        set x [get_a_x]
        set y [get_a_y]
        set dist [distance_xy $x $y $center_x $center_y]
...
for {set i 0} {$i < $opt(nfnode)} {incr i} {
    ...
    set xf [get_a_x]
    set yf [get_a_y]
    set dist [deploy_sensor xf yf $opt(spot_x) $opt(spot_y) \
        [expr $opt(radius_range) * $opt(d_0)]]
    set fdists($i) $dist
    $fnode($i) set X_ $xf
    $fnode($i) set Y_ $yf
    $fnode($i) set Z_ 0
